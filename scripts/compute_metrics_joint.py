...
from eval_utils import (
    ...
    load_config, load_data, get_crystals_list, get_crystals_list_lengths_angles, prop_model_eval, compute_cov)
...
class Crystal(object):
    ...
    def __init__(self, crys_array_dict):
        ...
        self.lattice = crys_array_dict['lattice']
...
class Crystal_lengths_angles(object):
...
class GenEval(object):
    ...
    def __init__(self, pred_crys, gt_crys, n_samples=10, eval_model_name=None):
        self.crys = pred_crys
...
class OptEval(object):
    ...
    def __init__(self, crys, num_opt=10, eval_model_name=None):
        """
...
def get_crystal_array_list(file_path, batch_idx=0):
    ...
    if 'input_data_batch' in data:
        ...
        if isinstance(batch, dict):
            ...
            true_crystal_array_list = get_crystals_list(
                batch['frac_coords'], batch['atom_types'], batch['lattice'], batch['num_atoms'])
        else:
            # true_crystal_array_list = get_crystals_list(
            #     batch.frac_coords, batch.atom_types, batch.lengths,
            #     batch.angles, batch.num_atoms)
...
def get_crystal_array_list_lengths_angles(file_path, batch_idx=0):
    data = load_data(file_path)
    crys_array_list = get_crystals_list_lengths_angles(
        data['frac_coords'][batch_idx],
        data['atom_types'][batch_idx],
        # data['lattice'][batch_idx],
        data['lengths'][batch_idx],
    ...
    if 'input_data_batch' in data:
        batch = data['input_data_batch']
        if isinstance(batch, dict):
            true_crystal_array_list = get_crystals_list_lengths_angles(
                batch['frac_coords'], batch['atom_types'], batch['lengths'],
                batch['angles'], batch['num_atoms'])
        else:
            true_crystal_array_list = get_crystals_list_lengths_angles(
...
def main(args):
    ...
    if 'gen' in args.tasks:
        ...
        with open (args.root_path + '/{}_generated_crystals.pickle'.format(args.label), 'wb') as f:
            pickle.dump(crys_array_list, f)
        ...
        gen_evaluator = GenEval(
            gen_crys, gt_crys, n_samples = args.n_samples, eval_model_name=eval_model_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--root_path', default='') #required=True)
