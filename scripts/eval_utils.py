...
from model.common.constants import CompScalerMeans, CompScalerStds
from model.common.data_utils import StandardScaler, chemical_symbols
from model.pl_data.dataset import TensorCrystDataset
from model.pl_data.datamodule import worker_init_fn
...
def load_model(model_path, load_data=False, testing=True, min_max_scaler = False):
    with initialize_config_dir(str(model_path)):
        cfg = compose(config_name='hparams')
        ...
        # model.num_atoms_prior = '/home/m/Thesis/cdvae/cdvae/data/perov_train_num_atoms.pickle'
        ...
        if min_max_scaler:
            model.lattice_min_max_scaler = torch.load(model_path / 'lattice_min_max_scaler.pt')
...
def get_crystals_list(
    ...
    for batch_idx, num_atom in enumerate(num_atoms.tolist()):
        ...
        crystal_array_list.append({
            ...
            'lattice': cur_lattice.detach().cpu().numpy(),
...
def get_crystals_list_lengths_angles(
        # frac_coords, atom_types, lengths, angles, num_atoms):
        frac_coords, atom_types, lengths, angles, num_atoms):
    """
    args:
...
def structure_validity(crystal, cutoff=0.5):
    ...
    if dist_mat.min() < cutoff or crystal.volume < 0.1:
        if crystal.volume < 0.1:
            print("Crystal volume < 0.1 ", crystal.volume) 
        else:
            print("Dist mat, cutoff", dist_mat.min(), cutoff)
