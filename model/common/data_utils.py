...
def build_crystal(crystal_str, niggli=True, primitive=False): #, primitive_standard=False):
    ...
    if primitive:
        ...
        # crystal = SGA.get_conventional_standard_structure()
        # print('refined')
        # crystal = SGA.get_refined_structure()
        crystal = SGA.get_primitive_standard_structure()
    ...
    #     crystal = crystal.get_primitive_standard_structure()
...
def lattice_matrix_to_params_torch(matrix):
    lengths = torch.sqrt(torch.sum(matrix ** 2, axis=1)).tolist()

    angles = torch.zeros(3)
    for i in range(3):
        j = (i + 1) % 3
        k = (i + 2) % 3
        angles[i] = abs_cap(torch.dot(matrix[j], matrix[k]) /
                            (lengths[j] * lengths[k]))
...
def frac_to_cart_coords_lattice_matrix(
    frac_coords,
    lattice,
    num_atoms,
):
...
class MinMaxScalerTorch(object):
    ...
    def transform(self, X):
        X = torch.tensor(X, dtype=torch.float)
        return (X - self.mins) / (self.maxs - self.mins + EPSILON)
    ...
    def __repr__(self) -> str:
        return (
            f"{self.__class__.__name__}("
            f"mins: {self.mins.tolist()}, "
            f"maxs: {self.maxs.tolist()})"
        )


def get_scaler_from_data_list(data_list, key, type=''):
...
def preprocess(input_file, num_workers, niggli, primitive,# primitive_standard,
                graph_method,
               prop_list):
    df = pd.read_csv(input_file)

    def process_one(row, niggli, primitive, graph_method, prop_list):# primitive_standard, graph_method, prop_list):
        crystal_str = row['cif']
        crystal = build_crystal(
            crystal_str, niggli=niggli, primitive=primitive)# , primitive_standard = primitive_standard)
        
        frac_coords = torch.Tensor(crystal.frac_coords)
        atom_types = torch.LongTensor(crystal.atomic_numbers)
        lattice = crystal.lattice.matrix.copy()
        lattice = torch.Tensor(lattice)
        lattice_parameters = torch.Tensor(crystal.lattice.parameters)
        lengths = lattice_parameters[:3]
        angles = lattice_parameters[3:]
        ...
        result_dict = {
            ...
            'lengths': lengths,
            'angles': angles,
            'num_atoms': np.array(atom_types).shape[0],
...
def add_scaled_lattice_prop(data_list, lattice_scale_method):
    for dict in data_list:
        ...
        ### scaling the matrix representation and not the lengths and angles
        lattice = lattice_params_to_matrix(*lengths, *angles)
        dict['lattice'] = lattice

# def add_min_max_lattice(data_list, lattice_scale_method):
#     for dict in data_list:
#         graph_arrays = dict['graph_arrays']
#         # the indexes are brittle if more objects are returned
#         lengths = dict['lengths']
